...
1. **「ファイル」** メニューの **「フォルダーを開く」** をクリックし、ラボ 5 の Starter フォルダーに移動します。

   ラボ 5 の Starter フォルダーは、ラボ 3 で開発環境をセットアップするときにダウンロードしたラボ リソース ファイルの一部です。フォルダ パスは次のとおりです。
...
1. 「**フォルダーを開く**」 ダイアログで、「**ContainerDevice**」 をクリックしてから、「**フォルダーの選択**」 をクリックします。
   ...
   ContainerDevice フォルダーは、ラボ 5 の Starter フォルダーのサブフォルダーです。Program.cs ファイルと ContainerDevice.csproj ファイルが含まれています。
