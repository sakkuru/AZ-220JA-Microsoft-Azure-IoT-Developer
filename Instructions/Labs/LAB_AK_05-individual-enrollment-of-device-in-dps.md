...
1. コード エディターで、`// 以下に Main メソッドを挿入します` コメントを見つけます。
...
1. `// 以下にセットアップ OnDesiredPropertyChanged イベント処理を挿入します` コメントを見つけます。
...
1. 「必要なツイン プロパティが変更されました: 」行の後の JSON 出力には、デバイスの 'telemetryDelay' に必要な値が含まれていることに注意してください。
...
1. シミュレートされたデバイス アプリケーションが Azure IoT Hub にテレメトリ イベントを送信し始めることに注意してください。
    ...
    テレメトリ イベントには、温度、湿度、圧力、緯度、および経度の値が含まれ、次のようになるはずです。
...
1. デバイスが、デバイス ツインプロパティの変更を認識することに注意してください。
    ...
    出力には、新しい必要な 'telemetryDelay' プロパティ値の JSON と共に、「必要なツイン プロパティが変更されました」というメッセージが表示されます。デバイスは、デバイス ツインの必要な状態の新しい構成を選択すると、現在構成されているように、5 秒ごとにセンサー テレメトリの送信を開始するよう自動的に更新されます。
