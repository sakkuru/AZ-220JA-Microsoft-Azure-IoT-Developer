...
| デバイス ID       | sensor-v-3000(対称キー)                |

必要に応じて、リソースの作成とデバイスの登録を行なってください。以下の手順を実行することでも作成と登録が可能です。

1. 「**Azure にデプロイする**」を選択します。

   [Azure にデプロイする](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FMicrosoftLearning%2FAZ-220-Microsoft-Azure-IoT-Developer%2Fbicep%2FAllfiles%2FARM%2Flab07.json)
...
IoT ソリューションでは、データの種類に応じて、またはビジネス上の理由から、受信メッセージ データを複数のエンドポイントの場所に送信する必要があることがよくあります。Azure IoT Hub は、ソリューションに必要な場所に受信データを送信できるようにする*メッセージ ルーティング*機能を提供します。
...
1. 左側のメニューの「**ハブの設定**」で、「**メッセージ ルーティング**」をクリックします。
...
1. 「**最小 TLS バージョン**で、「**Version 1.2**」が選択されていることを確認します。
...
1. **「Storage Explorer (プレビュー)」** ペインで、**BLOB コンテナー**を展開し、**「vibrationcontainer」** をクリックします。
   ...
   > **注**: データが表示されない場合は、しばらく待ってから再試行してください。
...
1. 「**ここをクリックしてダウンロードを開始する**」リンクをクリックします。

   **{day_num}.avro** (例: **22.avro**) という名前のファイルが**ダウンロード** フォルダーに、ダウンロードされます。

1. ダウンロードしたファイルを **Visual Studio Code** で開き、「**それでも開きますか**」をクリックします。

   データは読みやすい形式ではありませんが、振動メッセージとして認識できるはずです。

1. データを含む **Visual Studio Code** のドキュメントを閉じます。
...
#### タスク 2: Stream Analytics ジョブの入力を作成する
...
#### タスク 3: Stream Analytics ジョブの出力を作成する
...
1. 「**認証モード**」、「**接続文字列**」を選択します

   > **ストレージ アカウント キー**が表示されることに**注意**してください。
...
#### タスク 4: Stream Analytics ジョブのクエリを作成する
...
1. Stream Analytics ジョブの「**概要**」ペインで、「**開始**」をクリックします。

1. 「**ジョブの開始**」ペインで、「**ジョブ出力の開始時刻**」を「**現在**」のままにし、「**開始**」をクリックします。
...
1. ストレージ アカウントの「**概要**」ペインで、「**監視**」セクションを選択します。
...
1. **Storage Explorer (プレビュー)** の「**BLOB コンテナー**」で、「**vibrationcontainer**」をクリックします。

1. データを表示するには、json ファイルを選択して「**ダウンロード**」をクリックし、「**ここをクリックしてダウンロード**」をクリックします。
