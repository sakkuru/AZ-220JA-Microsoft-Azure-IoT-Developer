...
```
    ...
    このスクリプトは、まず **rg-az220** という名前のリソース グループ と **AZ-220-ハブ-{YourID}** という名前の IoT ハブを作成します。  既に存在する場合は、対応するメッセージが表示されます。次に、スクリプトは IoT ハブに 3 つのデバイスを追加し、デバイスの接続文字列を表示します。デバイス ID は次のとおりです。**TruckDevice**、**AirplaneDevice**、および **ContainerDevice**。   
...
1. 「**層**」 フィールドで、「**S1** 価格レベル」 が選択され、「**容量**」  が **1** に設定されていることを確認します。
...
1. 左側のナビゲーション メニューで、「**Analyze**」 が選択されていることを確認します。

    ナビゲーション メニューを展開すると、ボタン名を表示できます。「Analyze」と「Model」の 2 つのオプションがあります。「Analyze」 を選択します。

    ナビゲーション メニューを折りたたみ、ページの左側にクエリ編集領域が表示されるようにします。

1. クエリ エディターで 「**MEASURE**」 ドロップダウンを開き、「**temperature**」 をクリックします。

1. 「**SPLIT BY**」 ドロップダウンを開き、「**iothub-connection-device-id**」 をクリックします。

    クエリを実行すると、グラフが分割され、各 IoT デバイスからの製品利用統計情報がグラフに個別に表示されます。

1. **「Add」** をクリックします。

1. ページの上部で、表示を自動的に更新するには、**「Auto Refresh」** をクリックします。

    **「Auto Refresh」** が有効になっている場合、ディスプレイは _30 秒_ごとに更新され、最新のデータが表示されます。これは、利用可能なデータの過去 1 時間にのみ適用されます。
...
1. 2 番目のクエリをディスプレイに追加するには、「**MEASURE**」 ドロップダウンを 「**humidity**」 に設定し、「**SPLIT BY**」 ドロップダウンを 「**iothub-connection-device-id**」 に設定し、「**Add**」 をクリックします。
