...
1. Virtual machines**ページで、**+ Add**をクリックし、**Virtual machine**を選択します。

1. Create a virtual machine**ブレードで**、このコースで使用しているAzure Subscriptionを選択します。

1. 1. **リソースグループ**の右にある**新規作成**をクリックします。
...
1. Region**ドロップダウンで、Azure IoT Hubがプロビジョニングされている地域を選択します。
...
1. 最新バージョンの**IoT Edgeランタイム**をインストールするには、以下のコマンドを実行します。
    ...
    > **TIP**と入力してください。もし、`apt list -a iotedge`コマンドの出力に現れた以前のバージョン、例えば *1.0.9-1** をインストールしたい場合は、次のようなコマンドを使います。
    > ```bash
