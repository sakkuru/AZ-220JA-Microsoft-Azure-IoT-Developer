...
このエクササイズでは、Linux を使用してテスト証明書を生成します。この操作は、このコースのラボ 3 の間に作成したこのラボの "スターター" フォルダー内に含まれるヘルパー スクリプトを使用して、`vm-az220-training-gw0001-{your-id}` 仮想マシンで実行します。
...
1. Azure IoT Hub の接続文字列を使用して Edge デバイスを構成するには、次のコマンドを実行します。
    ...
    必ず `{iot-edge-device-connection-string}` プレース ホルダーを、IoT Edge デバイス用に以前コピーした接続文字列に置き換えてください。
