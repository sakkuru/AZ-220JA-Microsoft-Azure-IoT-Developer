...
1. **\<your device connection string\>** をsensor-th-0155のデバイス接続文字列に置き換えます。
...
1. 左側のナビゲーション メニューの「**Configurations + Deployments**」をクリックします。

1. 「**Configurations + Deployments**」ウィンドウで、「**+ Add**」をクリックし、、「**Device Twin Configuration**」を選択します。

1. 「**構成の作成**」ブレードの「**名前**」 で、**firmwareupdate** と入力します。

   「**ラベル** の下ではなく、構成に必要な「**名前**」フィールドに `firmwareupdate`と入力していることを確認 します。

1. ブレードの最下部で、「**次へ:**」をクリックします。**ツインの設定 >**。
...
1. ブレードの最下部で、「**次へ:**」をクリックします。
   「**確認および作成**」タブが開くと、新しい構成の 「検証に成功しました」というメッセージが表示されます。

1. 「**確認および作成**」タブで、「検証が成功しました」というメッセージが表示されたら、「**作成**」をクリックします。
