...
1. 接続文字列をテキスト ドキュメントにコピーし、**Ssensor-th-0155** デバイス用であることに注意してください。
...
1. 新しいコンソール アプリを作成するには、次のコマンドを入力します。
    ...
    > **注**: 新しい .NET コンソール アプリが作成されると、'dotnet 復元' は作成後のプロセスとして実行されている必要があります。[ターミナル] ペインに、この問題が発生したことを示すメッセージが表示されない場合、アプリは必要な .NET パッケージにアクセスできない場合があります。これに対処するためには、次のコマンドを入力します。`dotnet restore`
...
1. 次のコードをコピーして [コード エディター] ペインに貼り付けます。
    ...
    namespace fwupdatedevice
    {
        class SimulatedDevice
        {
            ...
            {
                LogToConsole (" + GetFirmwareVersion() + " to version " + fwVersion からファームウェアの更新が要求されました);
...
1. [ターミナル] ウインドウの内容を確認します。
    ...
    ``` bash
        mydevice: デバイスが起動されました
        mydevice: 現在のファームウェア バージョン: 1.0.0
    ```
