...
1. 「**Azure にデプロイする**」を選択します。
   ...
   [Azure にデプロイする](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FMicrosoftLearning%2FAZ-220-Microsoft-Azure-IoT-Developer%2Fbicep%2FAllfiles%2FARM%2Flab18.json)
...
1. 「**ワークスペースの構成**」 セクションの 「**接続する Log Analytics ワークスペースの選択:**」 で、トグル ボタンが 「**オン**」 にします。
...
1. 「**イメージ**」フィールドで、**Ubuntu Server 18.04LTS-Gen2** イメージを選択します。
...
1. 「**VM サイズの選択**」ブレードの「**VM サイズ**」で、「**Standard_DS1_v2**」、「**選択**」の順にクリックします。
   ...
   のサイズを選択するには、「**すべてのサイズを表示**」リンクを選択する必要がある場合があります。
...
1. 「フォルダーを開く」 ダイアログで、ラボ 18 の Starter フォルダーに移動します。
